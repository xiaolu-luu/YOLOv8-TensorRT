...
void Posetask::setup(void const* data, size_t size) {
   /*
...
bool Posetask::postprocess_cpu() {
    ...
    for (int i = 0; i < boxes_count; i ++){
        ...
        // for (int i = 0; i < 51; ++i) {
        //     std::cout << "mc_ptr[" << i << "] = " << mc_ptr[i] << std::endl;
        // }
        bbox yolo_box(x0, y0, x1, y1, conf, label);
        yolo_box.mc = cv::Mat(1, 51, CV_32F, mc_ptr);
    ...
    */
    ...
    for(int i = 0; i < m_bboxes.size(); i ++){
        ...
        // poseIn.push_back(m_bboxes[i].mc);
        poseIn = m_bboxes[i].mc;
        std::cout << "Size of poseIn: " << poseIn.rows << " rows, " << poseIn.cols << " cols" << std::endl;
        // for (int z = 0 ; z<poseIn.cols ;z+=3){
        //     std::cout<<poseIn.at<float>(0,z)<<" ";
        // }
        // std::cout<<std::endl;
        for (int z = 0 ; z<poseIn.cols ;z+=3){
            preprocess::affine_transformation(preprocess::affine_matrix.reverse, poseIn.at<float>(0,z), poseIn.at<float>(0,z+1), &poseIn.at<float>(0,z), &poseIn.at<float>(0,z+1));
        }
        // for (int z = 0 ; z<poseIn.cols ;z+=3){
        //     std::cout<<poseIn.at<float>(0,z)<<" ";
        // }
        // std::cout<<std::endl;
        m_bboxes[i].mc = poseIn;
     ...
     * 4. cv::putText
     */
    string tag   = "pose-" + getPrec(m_params->prec);
    ...
    for (int i = 0; i < final_bboxes.size(); i ++){
        ...
        auto pose_inf = box.mc;
        // for (int z = 0 ; z<pose_inf.cols ;z+=3){
        //     std::cout<<pose_inf.at<float>(0,z)<<" ";
        // }
        // std::cout<<std::endl;
        std::map<int,cv::Point> z_pose_map;
        int pose_num = 0;
        for (int z = 0 ; z<pose_inf.cols ; z+=3){
            
            if (pose_inf.at<float>(0,z+2) >= conf_threshold){
                cv::Point pose_pos(round(pose_inf.at<float>(0,z)),round(pose_inf.at<float>(0,z+1)));
                // std::cout<<pose_pos<<std::endl;
                z_pose_map[pose_num] = pose_pos;
                if (z <13){
                    cv::circle(m_inputImage,pose_pos,3,cv::Scalar(0,0,255),-1);    
                }
                else if (z<31 && z>=13){
                    cv::circle(m_inputImage,pose_pos,3,cv::Scalar(0,255,0),-1);
                }
                else{
                    cv::circle(m_inputImage,pose_pos,3,cv::Scalar(255,0,0),-1);
                }
        ...
        std::vector<std::pair<int, int>> connections_head = {
                {0, 1},
                {0, 2},
                {1, 3},
                {2, 4}
        ...
        for (const auto& conn : connections_head) {
            if (z_pose_map.count(conn.first) && z_pose_map.count(conn.second)) {
                cv::Point point0 = z_pose_map[conn.first];
                cv::Point point1 = z_pose_map[conn.second];
                cv::line(m_inputImage, point0, point1, cv::Scalar(255, 255, 0), 1);
            }
        }
        std::vector<std::pair<int, int>> connections_up = {
                {5, 6},
                {6, 8},
                {8, 10},
                {5, 7},
        ...
        for (const auto& conn : connections_up) {
            if (z_pose_map.count(conn.first) && z_pose_map.count(conn.second)) {
                cv::Point point0 = z_pose_map[conn.first];
                cv::Point point1 = z_pose_map[conn.second];
                cv::line(m_inputImage, point0, point1, cv::Scalar(255, 0, 255), 1);
            }
        }
        std::vector<std::pair<int, int>> connections_down = {
                {12, 14},
                {14, 16},
                {11, 13},
                {13, 15},
        ...
        for (const auto& conn : connections_down) {
            if (z_pose_map.count(conn.first) && z_pose_map.count(conn.second)) {
                cv::Point point0 = z_pose_map[conn.first];
                cv::Point point1 = z_pose_map[conn.second];
                cv::line(m_inputImage, point0, point1, cv::Scalar(0, 255, 255), 1);
            }
        }
        // if (z_pose_map.count(5) && z_pose_map.count(6)) {
        //     cv::Point point0 = z_pose_map[5];
        //     cv::Point point3 = z_pose_map[6];
        //     cv::line(m_inputImage, point0, point3, cv::Scalar(255, 0, 255), 1);
        //     }
...
bool Posetask::postprocess_gpu() {
    return postprocess_cpu();
}

shared_ptr<Posetask> make_posetask(
    std::string onnx_path, logger::Level level, Params params)
{
    return make_shared<Posetask>(onnx_path, level, params);
}
