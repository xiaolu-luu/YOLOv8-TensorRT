#ifndef __TRT_POSETASK_HPP__
#define __TRT_POSETASK_HPP__
...
class Posetask : public Model{

public:
    // 这个构造函数实际上调用的是父类的Model的构造函数
    Posetask(std::string onnx_path, logger::Level level, Params params) : 
...
// 外部调用的接口
std::shared_ptr<Posetask> make_posetask(
